...
// ---------------------------------------------------------------------------
// AccountsTabPanel — extracted from the former `/journal/accounts` route.
//
// As of the route-consolidation refactor, `/journal/accounts` is now a
// permanent redirect (`../accounts/page.tsx`) that lands on
// `/journal?tab=accounts`. The parent `/journal/page.tsx` mounts this
// panel when that query is present. The form + state machinery is a
// straight lift from the old route — only the outer page chrome was
// dropped because the parent page already provides
// `AmbientBackground` + padding.
// ---------------------------------------------------------------------------
...
export function AccountsTabPanel() {
