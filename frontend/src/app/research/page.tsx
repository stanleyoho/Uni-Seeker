...
export default function ResearchScreenerPage() {
  ...
  return (
    <div className="max-w-[1440px] mx-auto px-4 md:px-6 py-4 animate-fade-in">
      <h1 className="text-3xl font-bold mb-6 text-[var(--foreground)] tracking-tight">
        {t.screener.title}
      </h1>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-6">
        {PRESETS.map((preset) => (
          <button
            ...
            className="group flex items-center gap-3 p-4 text-left transition-all duration-200 hover:border-[var(--accent-primary)]/30 hover:bg-[var(--card-hover)]"
            style={{
              background: "var(--bg-secondary)",
              border: "1px solid var(--border-color)",
              borderRadius: 0,
          ...
          >
            <div className="text-[var(--accent-cyan)] group-hover:text-[var(--accent-primary)] transition-colors duration-200">
              {preset.icon}
            </div>
            ...
            {savedScreens.items.map((screen) => (
              <div key={screen.id} className="flex items-center gap-1">
                <button
                  ...
                  className="px-3 py-1.5 text-xs font-medium text-[var(--text-secondary)] hover:text-[var(--foreground)] hover:border-[var(--accent-primary)]/30 transition-all"
                  style={{
                    background: "var(--bg-secondary)",
                    border: "1px solid var(--border-color)",
                    borderRadius: 0,
      ...
      {/* Condition builder */}
      <GlassPanel title="篩選條件" className="mb-6">
        <ConditionBuilder
