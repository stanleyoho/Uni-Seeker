...
down_revision: Union[str, None] = '8bda05408c97'
...
def upgrade() -> None:
    ...
    sa.CheckConstraint('(account_id IS NOT NULL AND group_id IS NULL) OR (account_id IS NULL AND group_id IS NOT NULL)', name='ck_rule_one_owner'),
    sa.ForeignKeyConstraint(['account_id'], ['trade_accounts.id'], ),
    sa.ForeignKeyConstraint(['group_id'], ['account_groups.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_allocation_rules_account_id'), 'allocation_rules', ['account_id'], unique=False)
    op.create_index(op.f('ix_allocation_rules_group_id'), 'allocation_rules', ['group_id'], unique=False)
    ...
    sa.CheckConstraint('(account_id IS NOT NULL AND group_id IS NULL) OR (account_id IS NULL AND group_id IS NOT NULL)', name='ck_snapshot_one_owner'),
    sa.ForeignKeyConstraint(['account_id'], ['trade_accounts.id'], ),
    sa.ForeignKeyConstraint(['group_id'], ['account_groups.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_portfolio_snapshots_account_id'), 'portfolio_snapshots', ['account_id'], unique=False)
    op.create_index(op.f('ix_portfolio_snapshots_group_id'), 'portfolio_snapshots', ['group_id'], unique=False)
    ...
    op.create_index('ix_trades_account_symbol', 'trades', ['account_id', 'symbol', 'market', 'date'], unique=False)
    op.create_table('trade_lots',
    sa.Column('id', sa.Integer(), nullable=False),
    ...
    op.create_index('ix_trade_lots_fifo', 'trade_lots', ['account_id', 'symbol', 'market', 'is_exhausted', 'trade_id'], unique=False)
    op.create_index(op.f('ix_trade_lots_is_exhausted'), 'trade_lots', ['is_exhausted'], unique=False)
    # Partial unique indexes (cannot be expressed in SQLAlchemy __table_args__)
    op.execute("CREATE UNIQUE INDEX uq_account_snapshot ON portfolio_snapshots(account_id, date) WHERE account_id IS NOT NULL")
    op.execute("CREATE UNIQUE INDEX uq_group_snapshot ON portfolio_snapshots(group_id, date) WHERE group_id IS NOT NULL")
    op.execute("CREATE UNIQUE INDEX uq_rule_account_symbol ON allocation_rules(account_id, symbol) WHERE account_id IS NOT NULL")
    op.execute("CREATE UNIQUE INDEX uq_rule_group_symbol ON allocation_rules(group_id, symbol) WHERE group_id IS NOT NULL")
