...
class AlertService:
    ...
    async def update_rule(
        ...
        **fields: object,
        ...
        if (
            "threshold_type" in patch
            and patch["threshold_type"] not in ALERT_THRESHOLD_TYPES
        ):
        ...
        updated = await self._repo.update(
            rule_id, user_id=self._user.id, **patch
        )
        if updated is None:
            raise AlertRuleNotFound(f"rule {rule_id}")
        return updated
    ...
    async def _build_context(
        ...
        for p in enriched:
            ...
            if p.unrealized_pnl is not None:
                unr_pct = Decimal(
                    str(p.unrealized_pnl.unrealized_pnl_pct)
                ) * Decimal("100")
    ...
    def _format_trigger_message(rule: AlertRule, body: str) -> str:
        ...
        return (
            f"<b>Alert triggered</b>: {rule.name}\n"
            f"{body}\n"
            f"\n"
            f"管理規則: {link}"
        )
