...
class CompositeEstimator:
    ...
    async def calculate_and_save(self, stock_id: int) -> PriceEstimate | None:
        ...
        valid_results = [r for r in results if r.fair_price is not None and r.confidence > 0]
        ...
        comp_cheap = (
            sum((r.cheap_price or r.fair_price) * r.confidence for r in valid_results) / total_conf
        )
        comp_expensive = (
            sum((r.expensive_price or r.fair_price) * r.confidence for r in valid_results)
            / total_conf
        )
