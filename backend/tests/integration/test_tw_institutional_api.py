...
async def _seed(db: AsyncSession) -> dict[str, Stock]:
    ...
    rows = [
        TwInstitutionalNet(
            stock_id=stocks["2330"].id,
            date=d,
            foreign_net=10_000_000,
            trust_net=1_000_000,
            dealer_net=500_000,
            total_net=11_500_000,
        ),
        TwInstitutionalNet(
            stock_id=stocks["2454"].id,
            date=d,
...
async def test_top_net_empty_db_returns_clean_message(client: AsyncClient) -> None:
    ...
    resp = await client.get(
        "/api/v1/tw-institutional/top-net?date=2026-05-29&kind=foreign&direction=buy&limit=5"
    )
...
async def test_top_net_dealer_sell_orders_most_negative_first(
    ...
    resp = await client.get(
        "/api/v1/tw-institutional/top-net?date=2026-05-29&kind=foreign&direction=sell&limit=5"
    )
    assert resp.status_code == 200
    data = resp.json()["data"]
...
async def test_top_net_invalid_kind_rejected(client: AsyncClient) -> None:
    resp = await client.get("/api/v1/tw-institutional/top-net?kind=bogus&direction=buy&limit=5")
...
async def test_top_net_falls_back_to_latest_date_when_requested_missing(
    ...
    resp = await client.get(
        "/api/v1/tw-institutional/top-net?date=2026-05-30&kind=foreign&direction=buy&limit=5"
    )
    assert resp.status_code == 200
    body = resp.json()
...
async def test_symbol_history_returns_recent_rows(
    ...
    resp = await client.get("/api/v1/tw-institutional/symbol/2330?days=365")
    assert resp.status_code == 200
    body = resp.json()
