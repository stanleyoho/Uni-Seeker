...
async def _register_user(
    db: AsyncSession, email: str, username: str, password: str = "Password123"
) -> User:
...
async def test_first_login_registers_device_and_audits(
    ...
    devices = (await db_session.scalars(select(UserDevice).where(UserDevice.user_id == u.id))).all()
...
async def test_same_device_relogin_updates_last_seen_no_audit(
    ...
    audit_count = await db_session.scalar(
        ...
        .where(AuditLog.action == "device_added", AuditLog.user_id == u.id)
    )
...
async def test_fourth_device_blocked_with_device_limit_exceeded(
    ...
    assert (
        body.get("message") == "device_limit_exceeded"
        or body.get("detail") == "device_limit_exceeded"
    )
...
async def test_three_blocked_devices_do_not_count_against_active_limit(
    ...
    active = await db_session.scalar(
        ...
        .where(UserDevice.user_id == u.id, UserDevice.blocked_at.is_(None))
    )
    assert active == 1
