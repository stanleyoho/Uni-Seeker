...
@pytest.mark.asyncio
async def test_I01_happy_path_matched_pairs(
    ...
    await _post_trade(client, user, aid, "BUY", "AAPL", "100", "150",
                      "2024-01-05")
    await _post_trade(client, user, aid, "SELL", "AAPL", "100", "200",
                      "2024-07-01")
...
async def test_I02_free_tier_403_feature_unavailable(
    ...
    aid = await _create_account(client, user)
    await _post_trade(client, user, aid, "BUY", "AAPL", "10", "100",
                      "2024-01-05")

    with patch("app.services.portfolio.tax_report_service.settings") as s:
        s.enable_monetization = True
        r = await client.get(
            "/api/v1/holdings/exports/form8949.csv", headers=_auth(user)
        )
...
async def test_I03_basic_tier_403_feature_unavailable(
    ...
    with patch("app.services.portfolio.tax_report_service.settings") as s:
        s.enable_monetization = True
        r = await client.get(
            "/api/v1/holdings/exports/schedule_d.csv", headers=_auth(user)
        )
    assert r.status_code == 403, r.text
    assert r.json()["message"] == "feature_unavailable:tax_export"
...
@pytest.mark.asyncio
async def test_I04_pro_tier_passes(
    client: AsyncClient, db_session: AsyncSession
) -> None:
...
@pytest.mark.asyncio
async def test_I05_tax_year_filter(
    client: AsyncClient, db_session: AsyncSession
) -> None:
    ...
    aid = await _create_account(client, user)
    await _post_trade(client, user, aid, "BUY", "AAPL", "50", "100",
                      "2023-01-01")
    await _post_trade(client, user, aid, "SELL", "AAPL", "20", "120",
                      "2024-06-01")
    await _post_trade(client, user, aid, "SELL", "AAPL", "20", "130",
                      "2025-06-01")
    ...
    matches_2024, summary_2024 = await service.generate_form_8949(
        tax_year=2024
    )
...
@pytest.mark.asyncio
async def test_I06_account_id_filter(
    client: AsyncClient, db_session: AsyncSession
) -> None:
    ...
    await _post_trade(client, user, a1, "BUY", "AAPL", "10", "100",
                      "2024-01-01")
    await _post_trade(client, user, a1, "SELL", "AAPL", "10", "150",
                      "2024-06-01")
    await _post_trade(client, user, a2, "BUY", "MSFT", "5", "200",
                      "2024-02-01")
    await _post_trade(client, user, a2, "SELL", "MSFT", "5", "250",
                      "2024-07-01")
...
async def test_I07_form_8949_csv_layout_and_bom(
    ...
    await _post_trade(client, user, aid, "BUY", "AAPL", "10", "100",
                      "2024-01-01")
    await _post_trade(client, user, aid, "SELL", "AAPL", "10", "150",
                      "2024-06-01")
...
@pytest.mark.asyncio
async def test_I08_schedule_d_csv_layout(
    client: AsyncClient, db_session: AsyncSession
) -> None:
...
@pytest.mark.asyncio
async def test_I10_cross_user_isolation(
    client: AsyncClient, db_session: AsyncSession
) -> None:
    """User B cannot see user A's trades via tax export."""
    from app.services.portfolio import TaxReportService

    user_a = await _mk_user(db_session, "tax10a@x.tw", UserTier.PRO,
                             username="userA")
    user_b = await _mk_user(db_session, "tax10b@x.tw", UserTier.PRO,
                             username="userB")
    aid_a = await _create_account(client, user_a, name="A-account")
    await _post_trade(client, user_a, aid_a, "BUY", "AAPL", "10", "100",
                      "2024-01-01")
    await _post_trade(client, user_a, aid_a, "SELL", "AAPL", "10", "150",
                      "2024-06-01")
