...
def req(
    method: str, path: str, body: dict | None = None, auth: bool = False
) -> tuple[int, dict | str]:
    url = f"{BASE}{path}"
...
positions = (
    ...
    else (body.get("positions", []) if isinstance(body, dict) else [])
)
found_2330 = any(p.get("symbol") == "2330" for p in positions) if positions else False
check(
    "positions has 2330",
    status == 200 and found_2330,
    f"status={status} count={len(positions)} found={found_2330}",
)
...
check(
    "summary has total_cost",
    status == 200 and total_cost,
    f"status={status} total_cost={total_cost}",
)
...
check(
    "filers list has Berkshire", status == 200 and found_berk, f"status={status} count={len(subs)}"
)
